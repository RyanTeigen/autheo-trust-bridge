        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <CardContent className="p-4">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{auditStats.totalRecords}</p>
                  </div>
              ...
              <CardContent className="p-4">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{auditStats.verifiedRecords}</p>
                  </div>
              ...
              <CardContent className="p-4">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{auditStats.pendingVerification}</p>
                  </div>
            ...
            <Card className="bg-blue-50">
              <CardContent className="p-4">
                <div className="flex justify-between items-center">
                  <div>
                    <p className="text-sm text-muted-foreground">Blockchain Nodes</p>
                    <p className="text-2xl font-bold">{auditStats.blockchainNodes}</p>
                  </div>
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">Showing {getFilteredRecords().length} audit records</span>
            </div>
            <div className="flex items-center gap-2">
            ...
            <table className="w-full">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Resource</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Block Hash</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200">
                {getFilteredRecords().map((record) => (
                  <tr key={record.id} className={record.verified ? "" : "bg-amber-50"}>
                    <td className="px-4 py-4">
                      {record.verified ? (
                        ...
                        <Badge variant="outline" className="bg-amber-100 text-amber-800 flex items-center gap-1 border-0">
                    ...
                    <td className="px-4 py-4 font-medium">{record.action}</td>
                    <td className="px-4 py-4">{record.user}</td>
                    <td className="px-4 py-4">{record.resource}</td>
                    <td className="px-4 py-4 font-mono text-xs">{formatHash(record.hash)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
